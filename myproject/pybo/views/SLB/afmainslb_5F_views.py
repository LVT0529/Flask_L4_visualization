...
bp = Blueprint('afmainslb', __name__, url_prefix='/')

@bp.route('/afmainslb', methods=('GET', 'POST'))
@login_required
def _afmainslb():
    ...
    return render_template('slb_report/afmainslb.html', pool_data = pool_data, node_data = node_data)

@bp.route('/afmainslb/data', methods=('GET', 'POST'))
def _afmainslb_data():
    return render_template('slb_report/SLB_data.html')


@bp.route('/afmainslb/run_function', methods=['GET'])
def run_function():
